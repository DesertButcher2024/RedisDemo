...
- controller层： `com.hmdp.controller.UserController`
...
/user/me获取用户信息  `com.hmdp.controller.UserController`
...
// 4.保存验证码到redis,设置存活时间为2分钟
stringRedisTemplate.opsForValue().set(RedisConstants.LOGIN_CODE_KEY + phone, code,
        RedisConstants.LOGIN_CODE_TTL, TimeUnit.MINUTES);
```

2. 获取验证码改成从redis获取，登录信息改成存到redis中
...
`com.hmdp.controller.ShopController`

```java
@GetMapping("/{id}")
public Result queryShopById(@PathVariable("id") Long id) {
    return shopService.queryByById(id);
}
```
...
```java
public Result queryByById(Long id) {
